...
//        return pMinimap;
    ...
    uint32_t TiledMap::getTileAt(sTileLayer *pLayer, int x, int y)
    {
        if (x < 0 || y < 0 || x >= pLayer->width || y >= pLayer->height) return 0;

        auto i = (y * pLayer->width + x);
        return pLayer->tileIds[i];
    }

    void TiledMap::setTileAt(sTileLayer *pLayer, int x, int y, int tileId)
    {
        if (x < 0 || y < 0 || x >= pLayer->width || y >= pLayer->height) return;

        auto pInternalLayer = (sTileLayerInternal*)pLayer;
